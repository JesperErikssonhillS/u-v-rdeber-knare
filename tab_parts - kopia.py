...
class TabParts:
    def __init__(self, parent, app_data):
        ...
        self.layer_type_cb = ttk.Combobox(frame_bygg, textvariable=self.layer_type_var, state="readonly", width=38, 
                                          values=["Standardmaterial", "Regelverk / Blandat skikt", "Eget material", "Oventilerad luftspalt (Ange R-värde)", "Ventilerad luftspalt (R = 0)"])
    ...
    def edit_layer_mode(self):
        ...
        l_type = raw["type"]
        self.layer_type_var.set(l_type)
        self.on_layer_type_change(None)

        if l_type == "Standardmaterial":
            self.std_mat_var.set(raw["mat"])
            self.std_thick_var.set(raw["thick"])
        elif l_type == "Regelverk / Blandat skikt":
